types = {
    "t2.nano": {"CPU": 1, "RAM": .5},
    "t2.micro": {"CPU": 1, "RAM": 1},
    "t2.small": {"CPU": 1, "RAM": 2},
    "t2.medium": {"CPU": 2, "RAM": 4},
    "t2.large": {"CPU": 2, "RAM": 8},
    "t2.xlarge": {"CPU": 4, "RAM": 16},
    "t2.2xlarge": {"CPU": 8, "RAM": 32},

    "m4.large": {"CPU": 2, "RAM": 8, "SpotBidPrice": 0.05},
    "m4.xlarge": {"CPU": 4, "RAM": 16, "SpotBidPrice": 0.1},
    "m4.2xlarge": {"CPU": 8, "RAM": 32, "SpotBidPrice": 0.2},
    "m4.4xlarge": {"CPU": 16, "RAM": 64, "SpotBidPrice": 0.4},
    "m4.10xlarge": {"CPU": 40, "RAM": 160, "SpotBidPrice": 1.1},
    "m4.16xlarge": {"CPU": 64, "RAM": 256, "SpotBidPrice": 1.75},

    "m5.large": {"CPU": 2, "RAM": 8, "SpotBidPrice": 0.05},
    "m5.xlarge": {"CPU": 4, "RAM": 16, "SpotBidPrice": 0.11},
    "m5.2xlarge": {"CPU": 8, "RAM": 32, "SpotBidPrice": 0.25},
    "m5.4xlarge": {"CPU": 16, "RAM": 64, "SpotBidPrice": 0.45},
    "m5.12xlarge": {"CPU": 48, "RAM": 192, "SpotBidPrice": 1.4},
    "m5.24xlarge": {"CPU": 96, "RAM": 384, "SpotBidPrice": 3.0},

    "c4.large": {"CPU": 2, "RAM": 3},
    "c4.xlarge": {"CPU": 4, "RAM": 7},
    "c4.2xlarge": {"CPU": 8, "RAM": 15},
    "c4.4xlarge": {"CPU": 16, "RAM": 30},
    "c4.8xlarge": {"CPU": 36, "RAM": 60},

    "c5.large": {"CPU": 2, "RAM": 4},
    "c5.xlarge": {"CPU": 4, "RAM": 8},
    "c5.2xlarge": {"CPU": 8, "RAM": 16},
    "c5.4xlarge": {"CPU": 16, "RAM": 32},
    "c5.9xlarge": {"CPU": 36, "RAM": 72},
    "c5.18xlarge": {"CPU": 72, "RAM": 144},

    "r4.large": {"CPU": 2, "RAM": 15},
    "r4.xlarge": {"CPU": 4, "RAM": 30},
    "r4.2xlarge": {"CPU": 8, "RAM": 61},
    "r4.4xlarge": {"CPU": 16, "RAM": 122},
    "r4.8xlarge": {"CPU": 32, "RAM": 244},
    "r4.16xlarge": {"CPU": 64, "RAM": 488},
    "x1.16xlarge": {"CPU": 64, "RAM": 976},
    "x1.32xlarge": {"CPU": 128, "RAM": 1952},

    "x1e.xlarge": {"CPU": 4, "RAM": 122},
    "x1e.2xlarge": {"CPU": 8, "RAM": 244},
    "x1e.4xlarge": {"CPU": 16, "RAM": 488},
    "x1e.8xlarge": {"CPU": 32, "RAM": 976},
    "x1e.16xlarge": {"CPU": 64, "RAM": 1952},
    "x1e.32xlarge": {"CPU": 128, "RAM": 3904},

    "d2.xlarge": {"CPU": 4, "RAM": 30},
    "d2.2xlarge": {"CPU": 8, "RAM": 61},
    "d2.4xlarge": {"CPU": 16, "RAM": 122},
    "d2.8xlarge": {"CPU": 36, "RAM": 244},

    "h1.2xlarge": {"CPU": 8, "RAM": 32},
    "h1.4xlarge": {"CPU": 16, "RAM": 64},
    "h1.8xlarge": {"CPU": 32, "RAM": 128},
    "h1.16xlarge": {"CPU": 64, "RAM": 256},

    "i3.large": {"CPU": 2, "RAM": 15},
    "i3.xlarge": {"CPU": 4, "RAM": 30},
    "i3.2xlarge": {"CPU": 8, "RAM": 61},
    "i3.4xlarge": {"CPU": 16, "RAM": 122},
    "i3.8xlarge": {"CPU": 32, "RAM": 244},
    "i3.16xlarge": {"CPU": 64, "RAM": 488},

    "p2.xlarge": {"CPU": 4, "RAM": 61},
    "p2.8xlarge": {"CPU": 32, "RAM": 488},
    "p2.16xlarge": {"CPU": 64, "RAM": 732},

    "p3.2xlarge": {"CPU": 8, "RAM": 61},
    "p3.8xlarge": {"CPU": 32, "RAM": 244},
    "p3.16xlarge": {"CPU": 64, "RAM": 488},

    "g3.4xlarge": {"CPU": 16, "RAM": 122},
    "g3.8xlarge": {"CPU": 32, "RAM": 244},
    "g3.16xlarge": {"CPU": 64, "RAM": 488},

    "f1.2xlarge": {"CPU": 8, "RAM": 122},
    "f1.16xlarge": {"CPU": 64, "RAM": 976}
}
